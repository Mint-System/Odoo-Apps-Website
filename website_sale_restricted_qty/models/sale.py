...
class SaleInherit(models.Model):
    ...
    def check_min_order_qty(self):
        messages=[]
        for line in self.filtered(lambda l: l.product_uom_qty and l.product_id.sale_min_qty):
            rounding = line.product_uom.rounding            
            invalid_lines = line.filtered(lambda o: float_compare(o.product_uom_qty, o.product_id.sale_min_qty, precision_rounding=rounding) < 0)
            if invalid_lines:
                for line in invalid_lines:
                    messages.append(_('Minimum Order Quantity for %s should be %s.') % (line.product_id.name, line.product_id.sale_min_qty))
        if len(messages) > 0:
            message = '\n'.join(messages)
