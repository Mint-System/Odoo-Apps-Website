...
class WebsiteSale(WebsiteSale):
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        if not line:
            res.qcontext.update(min_qty=int(product.sale_min_qty))
        return res
    ...
    def get_product_min_order_quantity(self, cval, product_id, show_error=False):
        ...
        min_qty = product.product_tmpl_id.sale_min_qty if min_qty <= 1.0 else 1.0

        if not product.product_template_attribute_value_ids:
            min_qty = product.product_tmpl_id.sale_min_qty
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if order.id:
            line = order.order_line.filtered(lambda o: o.product_id.id == product.id) 
            if not line.id:
                if int(add_qty) < product.sale_min_qty and 'product' in request.httprequest.referrer.split('/'):
                    return request.redirect(request.httprequest.referrer.split('?')[0]+'/?min_order_error=1&min_qty=%d'%product.sale_min_qty)
                elif int(add_qty) < product.sale_min_qty:
                    add_qty = product.sale_min_qty
    ...
    #             line = order.order_line.filtered(lambda o: o.product_id.id == product.id) 
    #             if not line.id:
    #                 if int(add_qty) < product.sale_min_qty:
    #                     add_qty = product.sale_min_qty
...
#             if not line:
#                 if add_qty and add_qty < product.sale_min_qty or not add_qty:
#                     kw.update(add_qty=int(product.sale_min_qty))
